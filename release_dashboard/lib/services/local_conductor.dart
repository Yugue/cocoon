...
class LocalConductorService extends ConductorService {
  ...
  late final Directory _engineCheckoutDirectory = fs.directory(platform.environment['HOME']);
  late final Directory _frameworkCheckoutDirectory = fs.directory(platform.environment['HOME']);
  ...
  pb.ConductorState? get state {
    ...
    return generateConductorState(
        currentPhase: pb.ReleasePhase.APPLY_ENGINE_CHERRYPICKS,
        engineCherrypicksInConflict: true,
        frameworkCherrypicksInConflict: true);
  ...
  Future<void> createRelease({
    ...
    // _engineCheckoutDirectory = fs.directory(platform.environment['HOME']);
    // _frameworkCheckoutDirectory = fs.directory(platform.environment['HOME']);
  }
